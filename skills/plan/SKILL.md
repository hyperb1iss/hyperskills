...
Tests and reviews measure correctness; nothing downstream measures sprawl unless the plan gave it a fence. At each wave boundary, classify the branch diff by top-level path against the mission and ask which pieces prove the MVP, not which pieces merely exist:

```bash
git diff --name-only origin/${BASE:-main}...HEAD | awk -F/ '{print $1"/"$2}' | sort | uniq -c | sort -nr   # BASE = the PR's actual base ref
```

The next-spec-gap loop is the engine of unattended work and the engine of accidental empires; the non-goals fence from Phase 4 is what this check reads.
